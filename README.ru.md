...
Этот проект представляет собой мобильное и веб-приложение для работы с RBT. Помимо менеджера задач приложение поддерживает оперативную работу с камерами, домофонами, домами и квартирами. Оно создано с использованием **Ionic**, **Vue 3**, **TypeScript** и **Vite**. Также поддерживаются функции PWA, а **Capacitor** используется для интеграции с нативными функциями устройств.

### Управление объектами RBT

В авторизованной части доступны четыре основных раздела:

- **Заявки** — существующий интерфейс TT;
- **Устройства** — камеры и домофоны, поиск, состояние и фильтр проблемных устройств;
- **Квартиры** — поиск дома, квартиры, пользователи, RFID-ключи, домофоны, блокировка и код открытия;
- **Настройки** — профиль и параметры PWA.

Подробные API-зависимости и ограничения описаны в [docs/rbt-operations.md](docs/rbt-operations.md).
